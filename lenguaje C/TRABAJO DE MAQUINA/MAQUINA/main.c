...
/*A*/void CargaLista(lista *l,int *cant){
    ...
    system("cls");
    if(*cant==32){
        printf("No se pueden cargar mas Equipos\n");
        fflush(stdin);
    ...
    do
    {
        system("cls");
        fflush(stdin);
        printf("cuantos paises desea cargar?\n");
        printf("usted va cargando %d paises de 32\n",*cant);
    }
    while(1!=scanf("%d",&n) || (n<0 || n+*cant>32));}
    for(i=*cant; i<*cant+n; i++)
    {
        ...
        scanf("%[^\n]",PCF);
        MAYUS(PCF);
        while(0==controla_pais(PCF)){
            printf("Ingrese el Pais Correctamente\n");
            fflush(stdin);
            scanf("%[^\n]",PCF);
            MAYUS(PCF);
        }
        if(Buscarpais(l, PCF)==1)
        {
            printf("El Pais ya esta ingresado\n");
            i--;
        }
        else
        {
...
int controla_pais(char pais[]){
    mun paises[32];
    init_paises(paises);
    int i=0;
    for(i;i<32;i++){
        if(0==strcmp(pais,paises[i].P)){
            return 1;
        }
    }
    return 0;
...
/*G*/void modifica_punt_part(lista *l){
    ...
    printf ("Ingrese el pais que desea modificar\n");
    fflush(stdin);
    scanf("%[^\n]",ppais);
    MAYUS(ppais);
    if(0==Buscarpais(l,ppais))
    {
...
/*H*/void eliminar_equipo(lista *l,int *cant){
    ...
    printf ("Ingrese el pais que desea eliminar\n");
    fflush(stdin);
    scanf("%[^\n]",ppais);
    MAYUS(ppais);
    if(1==Buscarpais(l,ppais)){
...
/*N*/void partidos_perdidos(lista *l){
    ...
    printf ("Ingrese el pais que desea buscar\n");
    fflush(stdin);
    scanf("%[^\n]",ppais);
    MAYUS(ppais);
    if(1==Buscarpais(l,ppais))
